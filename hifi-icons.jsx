// hifi-icons.jsx — line-icon set + car renders
//
// Tutte le icone in viewBox 24x24, stroke-based. La stroke width si adatta
// alla direzione (1.7 per A "Casa" più morbida, 1.4 per B "Lustro" più sottile).

function Icon({ name, size = 20, color = 'currentColor', stroke, T }) {
  ...
  const paths = {
    ...
    eyeOff: <><path d="M17.9 17.9A10.6 10.6 0 0 1 12 20C5 20 1 12 1 12a18.5 18.5 0 0 1 5.1-5.9M9.9 4.2A10.9 10.9 0 0 1 12 4c7 0 11 8 11 8a18.5 18.5 0 0 1-2.2 3.2M1 1l22 22M14.1 14.1a3 3 0 1 1-4.2-4.2" {...p} /></>,
    ...
    arrowUpRight: <><path d="M7 17L17 7M7 7h10v10" {...p} /></>,
    ...
    trash: <><path d="M3 6h18M8 6V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6" {...p} /></>,
    ...
    snow: <><path d="M12 2v20M2 12h20M5 5l14 14M19 5L5 19" {...p} /></>,
    cloud: <><path d="M18 10a5 5 0 0 0-9.6-1.5A4 4 0 0 0 7 16h11a3 3 0 0 0 0-6z" {...p} /></>,
    home: <><path d="M3 11l9-8 9 8M5 9v11h14V9" {...p} /></>,
    apple: <><path d="M16 8.5c-1.4 0-2.6-.8-3-2-1-2 .2-4 2-4M9 9c-2.2 0-4 2-4 5 0 4 3 9 5.5 9 1.5 0 2-1 3.5-1s2 1 3.5 1c2.5 0 5.5-5 5.5-9 0-3-1.8-5-4-5-2.5 0-3 1-5 1s-2.5-1-5-1z" {...p} /></>,
    google: <><path d="M21 12.3c0-.7-.1-1.4-.2-2H12v3.8h5.1a4.4 4.4 0 0 1-1.9 2.9v2.4h3a9.2 9.2 0 0 0 2.8-7.1zM12 21c2.5 0 4.7-.8 6.2-2.2l-3-2.4a5.5 5.5 0 0 1-8.2-2.9H3.9v2.5A9 9 0 0 0 12 21zM6.9 13.5a5.4 5.4 0 0 1 0-3v-2.5H3.9a9 9 0 0 0 0 8zM12 6.5c1.4 0 2.7.5 3.6 1.4l2.7-2.7A9 9 0 0 0 3.9 8l3 2.5A5.4 5.4 0 0 1 12 6.5z" fill={color} stroke="none" /></>,
  };
  return (
...
// ─────────────────────────────────────────────────────────
// CarRender — illustrazione stilizzata (side-view)
// ─────────────────────────────────────────────────────────
function CarRender({ width = '100%', height = '100%', T, variant = 'sedan', tone, label }) {
  // tone: 'soft' colored bg | 'neutral' | 'dark'
  const bg = tone === 'dark' ? T.ink1 : tone === 'colored' ? T.accentSoft : T.surfaceAlt;
  const bodyColor = tone === 'dark' ? '#fff' : tone === 'colored' ? T.ink1 : T.ink1;
...
Object.assign(window, { Icon, CarRender });
